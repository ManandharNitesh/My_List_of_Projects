...
?>
  ...
  /* if 2 or more results only one when clicked if same date*/
    ...
    $sql ="SELECT * FROM hawan WHERE ID='$id'";
    ...
    if($queryResults > 0){
      while($row = mysqli_fetch_assoc($result)){
        ...
        ?>
            ...
            <img src="../7th SEM Project/Hawanimages/<?php echo $image;?> " class="img-responsive">
            </div><!--end of well-->
...
<?php
  function deletePhoto($conn){
    ...
    if(isset($_POST['del'])){
      ...
      $image=$_POST['image'];
      $file_to_delete ="../7th SEM Project/Hawanimages/$image";
 ...
 */     
      ...
      //create connnection to database then run query method on sql statement
            $sql="DELETE FROM hawan WHERE id='$id'";
