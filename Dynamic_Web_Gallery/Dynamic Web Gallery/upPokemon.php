...
<?php
	...
	if(isset($_POST['upload'])){
		...
		$target = "pokemon/".basename($_FILES['image']['name']);
		...
		$sql = "INSERT INTO pokemon(image,text)VALUES('$image','$text')";
		...
		if(move_uploaded_File($_FILES['image']['tmp_name'],$target)){
			header("Location:upPokemon.php?Uploaded");
		}else{
			header("Location:upPokemon.php?Sorry");
		}
		}else{
...
<div id="f">
<form method="POST" action="upPokemon.php" enctype="multipart/form-data">
...
<?php
			...
			while($row = mysqli_fetch_assoc($result)){
				...
				echo "<div id='img_div'>";
	 			echo "<img src='pokemon/".$row['image']."'>";
	 			echo "<p>".$row['text']."</p>";
...
<?php
	function deletePhoto($conn){
		...
		if(isset($_POST['del'])){
			...
			header('Location:uppokemon.php?deleted');
